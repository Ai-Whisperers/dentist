# Estructura Sitio Web — Ometz Dental

**Versión:** 1.0 — 28 jun 2026
**Marca comercial:** Ometz Dental (אומץ — "coraje" en hebreo)
**Profesional:** Dra. Gabriella González Pane
**Dominio tentativo:** ometzdental.com.py (registrar 2 años mínimo)
**Foco SEO inicial:** profilaxis bucodental (keyword secundaria: "limpieza dental")
**Idioma:** español (con secciones en inglés para expats)
**Mercados:** Asunción (Mburucuyá) + Luque (cuando se confirme segunda sede)
**Lo que NO ofrecemos:** endodoncia, cirugía, ortodoncia, implantes (se derivan).

---

## Filosofía de la web

> "Ometz — Coraje para tu boca."

**Principio:** Presencia digital = evidencia, no publicidad.
**No se hace:** marketing genérico, frases prohibidas del ADN ("carillas sin tallar", "sonrisa perfecta", "precio imperdible").
**Sí se hace:** explicar criterio clínico, mostrar el proceso, dar precio honesto.

---

## Mapa del sitio (sitemap)

```
/                          → Home
/profilaxis-bucodental     → Servicio destacado (SEO principal)
/segunda-opinion           → Servicio destacado (diferenciador)
/servicios                 → Listado completo
/filosofia                 → Quién soy + los 5 pilares
/precios                   → Pricing público
/contacto                  → WhatsApp + dirección + horarios
/en/...                    → Mirror en inglés (sólo Home, Servicios, Contacto, Segunda opinión)
```

---

## 1. HOME (`/`)

### Hero
- **H1 (ES):** Ometz. Odontología con coraje.
- **H1 (EN):** Ometz. Dentistry with courage.
- **Subtítulo:** "Ometz" significa coraje en hebreo. Coraje para pensar antes de tocar, conservar antes de reemplazar, y explicar antes de vender. Atención personalizada en Asunción.
- **CTA principal:** Agendar consulta por WhatsApp → [link directo a wa.me]
- **CTA secundario:** Conocer servicios

### Bloque "Qué es Ometz" (3-4 columnas)
- **Coraje para pensar.** Diagnóstico claro antes de cualquier procedimiento.
- **Coraje para conservar.** Si se puede arreglar, se arregla.
- **Coraje para explicar.** Sabés exactamente qué, por qué, y cuánto.
- **Coraje para derivar.** Endodoncia, ortodoncia, cirugía: te conecto con colegas de confianza.

### Bloque "Perfil del paciente" (a quién le sirve esto)
- Pacientes con tratamientos previos que no están seguros
- Pacientes que quieren planificar antes de actuar
- Pacientes con operatoria acumulada que necesitan criterio clínico
- Pacientes que priorizan la conservación sobre la intervención

### CTA Final
- Mensaje directo a WhatsApp con quick reply pre-armado: "Hola! Quiero agendar una consulta."

---

## 2. PROFILAXIS BUCODENTAL (`/profilaxis-bucodental`) ⭐ SEO principal

> **Nota SEO:** "Limpieza dental" sigue siendo el término de búsqueda más usado por los pacientes. En el H1 usamos "profilaxis bucodental" (término profesional correcto), pero el contenido, meta description y alt tags mencionan "limpieza dental" como sinónimo para capturar ambas búsquedas.

### Meta title
Ometz Dental — Odontología con Coraje | Profilaxis y Segunda Opinión en Asunción

### Meta description
Profilaxis bucodental profesional en Asunción. Limpieza dental con criterio: evaluación completa, ultrasonido, pulido y recomendaciones. Gs 280.000. Agenda por WhatsApp.

### H1
Profilaxis Bucodental en Asunción — Más que una "limpieza"

### H2 — ¿Qué es la profilaxis?
La profilaxis bucodental es el procedimiento profesional de limpieza profunda que elimina placa bacteriana, sarro y manchas que el cepillado diario no alcanza. Lo que la gente llama "limpieza dental" es en realidad un procedimiento clínico: no es cosmético, es prevención.

### H2 — ¿Qué incluye?
1. Evaluación clínica inicial
2. Remoción de sarro supragingival con ultrasonido
3. Limpieza subgingival según necesidad
4. Pulido coronario
5. Aplicación de flúor (cuando corresponde)
6. Recomendaciones personalizadas de higiene

### H2 — ¿Cada cuánto?
- Pacientes sin enfermedad periodontal: cada 6 meses
- Pacientes con antecedentes de periodontitis: cada 3-4 meses
- Pacientes con ortodoncia o prótesis: según indicación

### H2 — ¿Por qué importa?
La profilaxis regular no es "ir a higienizar" — es la intervención más barata que existe para evitar caries, enfermedad periodontal, y deterioro de trabajos existentes.

### H2 — Precio
- **Profilaxis estándar: Gs 280.000**
- Profilaxis con curetaje por cuadrante: Gs 400.000 (casos con enfermedad periodontal)

### H2 — ¿Tenés radiografías previas o un presupuesto de otro profesional?
Si tenés imágenes o un plan de otro consultorio, podés traerlos — la consulta de profilaxis se puede convertir en una segunda opinión formal sin costo extra si pedís cita ampliada.

### CTA
→ Agendar profilaxis por WhatsApp

---

## 3. SEGUNDA OPINIÓN (`/segunda-opinion`) ⭐ Diferenciador

### H1
Segunda Opinión Odontológica en Asunción — Decidí con información real

### H2 — ¿Cuándo pedir una segunda opinión?
- Te dijeron que necesitás un tratamiento importante (endodoncia, corona, extracción) y querés confirmar.
- Te cotizaron un plan de tratamiento que te parece caro o agresivo.
- Ya empezaste un tratamiento y querés asegurarte antes de continuar.
- Te hicieron operatoria "parche sobre parche" y querés un plan integral.

### H2 — ¿Qué incluye?
- Análisis de radiografías o tomografía previa
- Examen clínico completo
- Revisión del plan propuesto por otro profesional
- Plan alternativo documentado por escrito
- Derivación a especialista si corresponde (endodoncista, periodoncista, cirujano)

### H2 — Precio
- Consulta inicial: Gs 300.000
- Consulta extendida / Segunda opinión formal: Gs 400-600.000 (depende de la complejidad)
- Sesión de planificación compleja: Gs 800.000 (rehabilitación integral, multidisciplinaria)

### H2 — Confidencialidad
Lo que hablamos es estrictamente entre vos y yo. Si decidís no avanzar con mi propuesta, te entrego toda la documentación por escrito para que sigas con quien vos elijas.

### CTA
→ Pedir segunda opinión por WhatsApp

---

## 4. SERVICIOS (`/servicios`)

### H1
Servicios Odontológicos — Asunción

### Categoría 1 — Prevención
- Profilaxis bucodental (Gs 280.000)
- Profilaxis con curetaje (Gs 400.000 / cuadrante)
- Aplicación de flúor
- Sellantes

### Categoría 2 — Operatoria (restauraciones)
- Restauración simple (1 superficie)
- Restauración compuesta (2-3+ superficies)
- Restauraciones Clase I, II, III, IV, V
- Restauraciones directas e indirectas
- Incrustaciones (inlay, onlay, overlay)

> **Detalle técnico:** uso resina compuesta, ionómero vítreo, cerómero. El material depende del caso clínico.

### Categoría 3 — Prótesis
- Prótesis fija: coronas unitarias, puentes
- Prótesis removible: acrílica, flex (nylon), cromo-cobalto
- Prótesis sobre implantes (PSI)
- Provisionales

> **Materiales de corona:** zirconia, cerómero, porcelana libre de metal, metalocerámica. Indicación según caso.

### Categoría 4 — Estética
- Blanqueamiento domiciliario (Gs 1.200.000 / arco)
- Blanqueamiento en consultorio (Gs 1.800.000 / arco)
- Blanqueamiento combinado
- Restauraciones estéticas

### Categoría 5 — Planificación
- Consulta inicial (Gs 300.000)
- Segunda opinión (Gs 400-600.000)
- Planificación integral compleja (Gs 800.000)
- Estudio y derivación multidisciplinaria

### Lo que NO hago (con honestidad)
- Endodoncia → derivo a endodoncista de confianza
- Cirugía bucal compleja → derivo a cirujano maxilofacial
- Ortodoncia → derivo a ortodoncista
- Implantes (parte quirúrgica) → derivo, pero realizo la PSI (prótesis sobre implantes)

### CTA
→ Consultar disponibilidad y precio por WhatsApp

---

## 5. FILOSOFÍA (`/filosofia`)

### H1
Cómo trabajo — Los 5 principios

1. **Pienso antes de tocar.** Cada caso empieza con un diagnóstico detallado.
2. **Conservo antes de reemplazar.** Si un diente se puede restaurar, se restaura.
3. **Explico antes de vender.** Sabés exactamente qué te voy a hacer, por qué, y cuánto cuesta.
4. **Veo personas antes que dientes.** Tu historia clínica importa más que tu radiografía.
5. **Derivo cuando corresponde.** Si tu caso necesita un especialista, te conecto con uno de confianza.

### Sección "Mi enfoque con pacientes traumados"
Si venís con la boca "hecha mierda", con parches por todos lados, con tratamientos acumulados que no resolvieron nada — ese es exactamente el tipo de caso que más me interesa. Mi ventaja no es la técnica aislada, es el criterio acumulado de haber visto muchos casos como el tuyo.

### Sección "Mi tecnología"
- Radiografía digital
- Laboratorios digitales (coronas fresadas, diseño CAD/CAM)
- Materiales de última generación

---

## 6. PRECIOS (`/precios`)

> **Disclaimer:** Los precios son referencia. El presupuesto final depende del caso clínico y se entrega por escrito después de la consulta inicial.

### Consultas
| Servicio | Precio (Gs) |
|---|---|
| Consulta inicial (30 min) | 300.000 |
| Consulta extendida (45-60 min) | 400.000 |
| Segunda opinión escrita | 450.000 |
| Segunda opinión + plan | 600.000 |
| Planificación integral compleja | 800.000 |

### Profilaxis
| Servicio | Precio (Gs) |
|---|---|
| Profilaxis estándar | 280.000 |
| Curetaje por cuadrante | 400.000 |
| Pulido / flash | 150.000 |

### Blanqueamiento
| Servicio | Precio (Gs) |
|---|---|
| Domiciliario (por arco) | 1.200.000 |
| En consultorio (por arco) | 1.800.000 |
| Combinado | 2.500.000 |

> **Restauraciones, prótesis, pernos:** Cada caso se presupuesta después de la consulta. La variabilidad es alta (material, complejidad, número de piezas). Pedí tu presupuesto personalizado por WhatsApp.

### Métodos de pago
- Efectivo (2-5% descuento)
- Transferencia (Itaú, Banco Familiar)
- Bancard POS (3, 6, 12 cuotas — paciente absorbe comisión)
- Pagopar (6, 12, 18 cuotas — paciente absorbe comisión)

---

## 7. CONTACTO (`/contacto`)

### H1
¿Hablamos?

### Bloque directo
- **WhatsApp:** [link wa.me con mensaje pre-armado]
- **Dirección:** Barrio Mburucuyá, Asunción (calle exacta a completar)
- **Horarios:** Lunes a viernes, con cita previa
- **Idiomas:** Español, Inglés

### Mensaje pre-armado para WhatsApp
"Hola Dra. Gabriella! Quiero agendar una consulta. Mi nombre es [_____] y me interesa [____]."

### Urgencias
Para urgencias fuera de horario: [número personal a designar]

---

## Sitemap secundario (Fase 2)

- `/casos-clinicos` — Casos reales documentados (con consentimiento), uno por mes
- `/blog` — Contenido educativo (sólo si se sostiene en el tiempo)
- `/sobre-mi` — Bio profesional extendida + credenciales
- `/preguntas-frecuentes` — FAQ con tono Gabriela
- `/red-derivacion` — Especialistas aliados con quienes trabajo

---

## Tracking

- Google Analytics 4
- Google Search Console
- Evento WhatsApp click → contactar a [Kiki/Iván] para definir KPI
