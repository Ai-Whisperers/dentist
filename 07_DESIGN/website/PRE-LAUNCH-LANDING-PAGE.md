# Landing Page Pre-Lanzamiento — Carrd.co
## Para usar ANTES del sitio principal
### 12 de Junio 2026

---

## OBJETIVO

Mientras se construye el sitio principal, esta landing page:
1. Captura interes de pacientes potenciales
2. Comunica que la practica esta por abrir
3. Recolecta emails/WhatsApp de personas interesadas
4. Proyecta profesionalismo

---

## CONTENIDO DE LA LANDING PAGE (carrd.co)

### URL propuesta
dra-gabriela-gonzalez.com.py (o .com)

### Estructura (1 pagina, scroll simple)

**Seccion 1 — Hero**

**Headline:**
Odontología con criterio. No con prisa.

**Subheadline:**
Apertura: [MES] 2026 en [BARRIO/LUQUE]. Segunda opinion, planificacion de tratamiento,
y atencion dental para personas que quieren entender sus opciones antes de decidir.

**CTA:**
Escribeme por WhatsApp para agendar

[Boton: Agendar consulta → wa.me/...]

---

**Seccion 2 — Quien soy**

Dra. Gabriella Gonzalez Pane
Odontologa | 20 años de experiencia
Ex-coordinadora Odontologia 3

Despues de dos decadas en consulta, abri mi practica privada con una filosofia simple:
planificar antes de actuar, preservar lo que existe, y explicar todo antes de decidir.

---

**Seccion 3 — Servicios (preview)**

- Segunda opinion
- Planificacion de tratamiento
- Odontologia general
- Estetica dental
- Rehabilitacion oral

[Link: Ver todos los servicios → se llena al lanzar el sitio]

---

**Seccion 4 — Por que elegir esta practica?**

- 20 años de experiencia clinica
- Filosofia conservadora: no hago trabajo innecesario
- Bilingue: español + ingles
- Precios transparentes publicados
- Plan de tratamiento escrito antes de cualquier procedimiento

---

**Seccion 5 — Contacto**

WhatsApp: +595 XXX XXX XXX (escribeme)
Email: dra.gp.odontologia@gmail.com
Ubicacion: [BARRIO], [CIUDAD], Paraguay
Horario: Solo con cita previa

---

**Seccion 6 — Call to action final**

**Headline:**
Tenes un caso que necesita segunda opinion?
No esperes.

[Boton: Escribime por WhatsApp]

---

## SETUP EN CARRD.CO

1. Crear cuenta en carrd.co
2. Elegir template Profile (1 pagina, scroll simple)
3. Copiar contenido arriba
4. Configurar WhatsApp button:
   - Link: https://wa.me/595XXXXXXXX?text=Hola%20Dra.%20GP...
5. Custom domain (opcional): dra-gabriela-gonzalez.com.py
   - Costo dominio: Gs 100-200k/año (.com.py) o Gs 50k/año (.com)
   - Carrd Pro para custom domain: /año

## PRESUPUESTO LANZAMIENTO

| Item | Costo |
|------|-------|
| Dominio .com.py | Gs 100-200k/año |
| Carrd Pro | ~Gs 70k/año () |
| Total | ~Gs 200-300k/año |

---

## MENSAJES DE WHATSAPP

### Mensaje de Bienvenida (auto-respuesta)
Hola! Soy la Dra. Gabriella. Gracias por escribir. En este momento estoy aceptando
pacientes para [MES]. Para agendar: (1) que te gustaria resolver? (2) tienes preferencia
de horario? Respondo en menos de 24 horas.

### Mensaje de Follow-up (24 horas)
Hola [NOMBRE], soy la Dra. Gabriella. Recibi tu mensaje. Te puedo ayudar con algo
especifico? Si tenes radiografias recientes o fotos, podes mandarlas por aca.

---

## ANALYTICS A CONFIGURAR

- [ ] Google Analytics 4
- [ ] WhatsApp click tracking
- [ ] Carrd analytics

---

*Plan: 12 de Junio 2026*
