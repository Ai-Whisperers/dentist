> **PRICING CROSS-REFERENCE:** All prices reference `00_STRATEGIC/financial-pricing/canonical-pricing-reference-v2.md`. When in doubt, the canonical doc wins.

# PHASE 0 CHECKLIST - Dra. GP Project
## This Week (June 4-13, 2026) - 5 Days
### The critical unblock. If this is not done, nothing else moves.

**For Ivan (orchestrator) - 9 hours total:**
**For Dra. Gabriella Gonzalez Pane (client) - 8 hours total**

---

## MONDAY (Day 1) - 2 hours

### Dra. GP (5 min) - ANSWER THESE 2 QUESTIONS:
1. **Do you have Gs 20M+ in savings?** (the #1 viability filter)
2. **What did Roque say in the meeting?** (or: he hasnt met with you yet)

### Ivan (1.5 hours) - PHONE CALLS:

**3 calls to Luque landlords
- [ ] | Luque landlord calls | Ivan | 2026-06-09 | 02_MEETINGS/client-prep/roque-meeting/luque-space-shortlist-3-priorities.md + phone log | | (30 min total):**
1. (021) 412-XXXX - San Lorenzo equipped consultorio, ask: still available? Gs 3M confirmed? Habilitacion current?
2. (021) XXX-XXXX - Local Comercial Centro #1, ask: rent + equipment availability
3. (021) XXX-XXXX - Local Comercial Centro #2, ask: rent + equipment availability

**4 calls to dental labs
- [ ] | 4 dental lab calls | Ivan | 2026-06-09 | 01_RESEARCH/procurement/MASTER-PROCUREMENT-GUIDE.md + call log | |
- [ ] | Dental lab calls | Ivan | 2026-06-09 | 01_RESEARCH/procurement/MASTER-PROCUREMENT-GUIDE.md + call log | | (60 min total):**
1. (021) 412 9900 - Alta Gama - WhatsApp first if no answer
   Ask: prices for PFM crown, zirconia monolithic, zirconia layered, e.max crown, veneer, 3-unit bridge, complete denture, night guard, turnaround, volume discount
2. 0981 392334 - Lab Dental AM Antonio Morinigo - same questions
3. 0972 720777 - Lab Dental Design CAD/CAM - same questions
4. 0981 644393 - LaboDent Digital (CDE) - same questions

Use the LAB-MYSTERY-SHOP-CALL-KIT.md script in 01_RESEARCH/procurement/ for exact questions.

**Document results in: 02_MEETINGS/client-prep/roque-meeting/luque-space-shortlist-3-priorities.md**

---

## TUESDAY (Day 2) - 3 hours

### Dra. GP (3-4 hours) - VISIT SET:
- Bring: CI, proof of address, RUC (if available)
- Tasks: Get CI certified copies, apply for RUC
- [ ] | CI/RUC + EAS | Dra. GP | 2026-06-10 | CI copies + RUC file + EAS status | | for EAS, ask about Timbrado process
- Online while there: Start EAS application
- [ ] | EAS application | Dra. GP | 2026-06-09 | EAS receipt / submitted status | | at eas.mic.gov.py (need RUC + CI)
- Timeline: 2-4 weeks for EAS approval

### Ivan (in parallel, 1 hour) - WHILE WAITING:
- Build specialist referral list
- [ ] | Specialist referral list | Ivan | 2026-06-10 | 01_RESEARCH/procurement/SPECIALIST-REFERRAL-LIST.md | |
- [ ] | Specialist referral list | Ivan | 2026-06-10 | 01_RESEARCH/procurement/SPECIALIST-REFERRAL-LIST.md | | (1 page printable) - SEE SPECIALIST-LIST.md in 01_RESEARCH/procurement/
- Identify 1-2 specialists per category: periodontist, endodontist, implantologist
- Get their phone numbers from COP directory or Google Maps

---

## WEDNESDAY (Day 3) - 3.5 hours

### Ivan (3.5 hours) - SUPPLIER VISITS:

**Visit 2-3 equipment suppliers
- [ ] | 2-3 equipment supplier visits | Ivan | 2026-06-10 | quotes in master guide | |
- [ ] | Equipment supplier visits | Ivan | 2026-06-10 | quote sheets in master guide | | in Asuncion:**
1. Dental Guarani - Yegros 1094 - ask for complete chair + autoclave + compressor package quote
2. Andres H. Arce (Tienda AHA) - ask for same package
3. Optional: Dent-Par, DEI Import, or Medsupar for comparison

### Ivan (in parallel, 1 hour) - PATIENT CONVERSATIONS:
- WhatsApp 5 current Odontologia 3 patients
- [ ] | 5 patient interviews | Ivan | 2026-06-10 | 02_MEETINGS/client-prep/data-collection/patient-survey-instrument.md responses | |: ask if they would consider private-pay service at higher price for no-overtreatment + planning-first approach
- Document responses in 02_MEETINGS/client-prep/data-collection/patient-survey-instrument.md

---

## THURSDAY (Day 4) - 1.5 hours

### Dra. GP (1 hour) - ROQUE CONVERSATION:
- 1 hour sit-down with Roque (or send message if hes remote)
- Goal: get answer on deal improvement
- Outcome: Option A (real improvement) / Option B-slow (token) / Option B-full (nothing) / unclear
- **CRITICAL: this is the single most important conversation in the entire project**

### Ivan (30 min) - UPDATE FINANCIAL MODEL:
- Add the real OpEx numbers from this week: rent quote, lab quote, supplier quote
- Update financial-model-projections-v2.md line 138-148 with actual runway numbers
- Fill in the blank: "INITIAL: Gs ___M needed" with Dra. GPs savings number

---

## FRIDAY (Day 5) - 3.5 hours

### Dra. GP (30 min) - COP MEMBERSHIP:
- Sign up for COP (Circulo Odontologico del Paraguay) - Gs 400,000/year
- Gral. Diaz 980 c/ Colon, Asuncion. Phone: (021) 493-296
- Benefits: radiology discounts, Clinica Social access, specialist network, legal advice

### Ivan (1.5 hours) - WEEKLY CLEANUP:
- Update master launch roadmap to reflect THIS WEEKS decisions
- Update financial model with real numbers
- [ ] | Financial model update | Ivan | 2026-06-12 | 00_STRATEGIC/financial-pricing/financial-model-projections-v2.md actuals | |
- Update procurement master guide with real quotes
- Update README.md to match actual 178 active markdown documents

### Ivan (15 min) - DECIDE:
- Corporate program scope: Option A (1 client Y1) / Option B (5-10 hrs/week for 6 mo) / Option C (delete)
- Default: Option A (RECOMMENDED for solo dentist)

### Ivan (1.5 hours) - 3-OPTION RECONCILIATION:
- Update three-strategic-options-analysis.md to recommend "B from day 1" (matches reality)
- Master launch roadmap is B. Three-strategic-options is A-B-C. Reconcile.

---

## EXIT CRITERIA (end of Friday June 13, 2026)

CHECK ALL 6:

- [ ] Roque result known (recorded in roque-meeting-results.md)
- [ ] Savings number known (Gs 20M+?)
- [ ] 3 landlord quotes received (in luque-space-shortlist-3-priorities.md)
- [ ] 4 lab quotes received (in master guide update)
- [ ] 1 supplier visit done (quote in master guide)
- [ ] Specialist referral list built (1 page printable)
- [ ] EAS application submitted
- [ ] RUC application started
- [ ] COP membership active
- [ ] 3-option phasing reconciled

**If 7+ are checked: proceed to Phase 1 (LEGAL + BANKING, Week 2-3)**
**If less than 7: extend Phase 0 by 1 week. Do not proceed.**

---

## QUICK REFERENCE - KEY PHONE NUMBERS

### Landlords:
- San Lorenzo equipped consultorio: see luque-rental-market-verified.md for contact
- 2 backup landlords: from same doc

### Dental Labs:
- Alta Gama: (021) 412 9900
- Lab Dental AM: 0981 392334
- Lab Dental Design CAD/CAM: 0972 720777
- LaboDent Digital (CDE): 0981 644393

### Equipment Suppliers:
- Dental Guarani: +595 21 494 411
- Andres H. Arce Tienda AHA: tiendaaha.com.py
- Dent-Par: dentpar.com.py

### Legal/Institutional:
- COP: (021) 493-296 - info@cop.com.py
- EAS Portal: eas.mic.gov.py
- SET RUC: servicios.set.gov.py
- MSPBS Habilitacion: MSPBS Central, Asuncion

### Tech/Payments:
- DenPro: denpro.com.py (30-day free trial)
- Pagopar: pagopar.com (no monthly fee)
- Bancard: bancard.com.py / 021-417-8000

---

## EMERGENCY CONTACTS

If blocked on anything, WhatsApp Ivan (the orchestrator).
This plan is owned by Ivan. Dra. GP and Ivan are the only 2 people who can execute Phase 0.

---

*Checklist generated: June 4, 2026*
*For: Ivan (autistic, terse) and Dra. Gabriella Gonzalez Pane*
*Owner: Sisyphus*
---

## TODO LINK (auto-added)
This checklist feeds TODO `P2.2 Phase 0 checklist`.
