# CLIENT ANALYSIS — ACTUALIZADO
## Dra. Gabriela González Pane

## PRICING CROSS-REFERENCE (June 2026)

> Service prices in this document are NOT authoritative. The master reference is:
> `00_STRATEGIC/financial-pricing/canonical-pricing-reference-v2.md`
>
> Any price update should happen in the canonical file only.

## ADN CROSS-REFERENCE (28 jun 2026)

> Este perfil de cliente se alinea con `ADN_Profesional_Dra_Gabriela_Gonzalez_Pane_EXTENDIDO.pdf`. La filosofía clínica, valores y visión expresados en este documento reflejan los 5 pilares del ADN: pensar antes de tocar, conservar antes que reemplazar, explicar antes de vender, ver personas antes que dientes, derivar cuando corresponde.

---

**Ultima actualización**: 28 junio 2026 (alineación con ADN Profesional)
**Status**: Con datos reales de los audios del 22 jun + ADN como base de filosofía

---

## 1. PROFILE DATA

| Campo | Valor |
|---|---|
| Nombre | Dra. Gabriela González Pane |
| Profesión | Odontóloga especialista en Rehabilitación Oral, Estética y Operatoria Dental |
| Trayectoria | ~20 años (graduada 2005, Universidad Autónoma del Paraguay) |
| Ubicación actual | Trabaja en Asunción, vive en Luque |
| Workplace actual | Odontología 3 (~13 años) |
| Status legal real (v2) | **Trabajadora independiente** que factura por procedimiento. NO es empleada. |
| Volumen pacientes | ~130 pacientes/mes en O3 |
| Restauraciones solo Mayo 2026 | **42** |
| Pacientes por tarde | **10** (vs. promedio odontólogo = 2/día) |
| Idiomas | Español (paraguayo), English |
| Edad | ~45 años (según audio: "tengo cuarenta y cinco") |

---

## 2. DATOS FINANCIEROS REALES (DEL AUDIO)

### Insurance Model — Los números rojos:

| Concepto | Valor | Fuente |
|---|---|---|
| Paciente paga al seguro/mes | Gs 500.000 | Audio: "el paciente le paga quinientos mil al seguro" |
| Seguro le paga a la clínica | ~10-15% | Audio: "nos paga a nosotros el diez, quince por ciento" |
| Clínica le paga a ella por restauración | **Gs 60.000** | Audio: "nos pagan sesenta mil" |
| Arancel SOP (precio "correcto") | Gs 1.500.000 | Audio: "el arancel del círculo es un millón y medio" |
| Precio que debería cobrar private | **Gs 400-550.000** | Audio + contexto |
| Precio que cobra chains/competitors | Gs 180-450.000 | Audio: "ciento ochenta a trescientos mil guaraníes por caries pequeña" |

**Conclusión: Por cada restauración de Gs 550k, recibe Gs 60k = 11% del valor real.**

### Estructura de Revenue Actual (Estimado):

| Procedimiento | Volume/mes | Pago clínica | Valor real | Gap |
|---|---|---|---|---|
| Restauraciones | ~40-42 | Gs 60k | Gs 550k | -490k |
| Profilaxis | ~12 | Gs 50k? | Gs 200k? | -150k |
| Perno | ~2 | ? | Gs 800k? | ? |
| Incrustaciones | ~3 | ? | Gs 1.200k? | ? |
| Coronas/puentes | ~2+ | ? | Gs 2.000k? | ? |

### Costos que soporta:

| Costo | Valor | Note |
|---|---|---|
| Materiales premium | Gs 500k/tubo resina alemán | Audio: "compraba los materiales más lindos" |
| Combustible Luque→Asunción | Gs 60k/día (~Gs 1M/mes) | Audio: "me gasto una fortuna en combustible" |
| Tiempo no cobrado | Alto | Casos complejos no pagan más |

---

## 3. FILOSOFÍA CLÍNICA — ADN-ALIGNED

> Audio: *"Para mí es muy sencillo agarrar y meter un poco de turbina y se va a conducto eso. Yo voy a cobrar mucho más, pero yo no voy a hacer eso porque mi ética profesional no me lo permite."*

Los 5 principios del ADN Profesional que Gaby sostiene en su práctica diaria:

### 1. Pensar antes de tocar
- Diagnóstico, conversación, lectura clínica, planificación y criterio antes de cualquier intervención
- No se apura a mostrar procedimientos; muestra cómo se decide si un procedimiento tiene sentido

### 2. Conservar antes que reemplazar
- Respeto por la estructura dentaria
- Una corona, una carilla o una rehabilitación no son trofeos. Son decisiones con costo biológico
- Audio: rechazo explícito al sobretratamiento ("no voy a hacer eso porque mi ética profesional no me lo permite")

### 3. Explicar antes de vender
- El paciente debe entender. Obligación ética y estrategia de diferenciación
- Educar sin infantilizar

### 4. Ver personas antes que dientes
- Capacidad explicativa superior al promedio
- Empatía con miedo, presupuesto, historia y frustración del paciente
- "Me colgó mucho así con estos casos... Mientras más desastres sean para mí mejor. ¿Por qué? Porque es más desafiante."

### 5. Derivar cuando corresponde
- "No existe un todólogo" — derivación adecuada cuando el caso lo requiere
- Madurez profesional, no ego

### Lo que dice sobre su trabajo:

> Audio: *"Me colgó mucho así con estos casos... Mientras más desastres sean para mí mejor. ¿Por qué? Porque es más desafiante."*

---

## 4. SITUACIÓN LABORAL ACTUAL

### Status legal real (v2 — CRÍTICO)

> **Gaby NO es empleada de O3.** Es **trabajadora independiente** que factura por procedimiento. Antes de este año tenían un fijo en Asismed, pero eso se cambió.

### Implicaciones críticas (cambian TODO el análisis):

| Aspecto | Realidad v2 |
|---|---|
| Antigüedad laboral | 13 años (comercial, no legal) |
| Derecho a indemnización si la echan | **NO existe** (no es empleada) |
| Tiene IPS | **NO** ("no tenemos un IPS") |
| Tiene aguinaldo/vacaciones | NO |
| Derecho a preaviso de 30 días | NO (no hay relación de dependencia) |
| Derecho a non-compete / exclusividad | Probable NO (depende del contrato de servicios) |
| Libertad para abrir práctica propia | **SÍ, sin pedir permiso** |
| Su "despido de facto" | Que dejen de derivarle pacientes sin aviso |

**Para Gaby:** esto es MEJOR (más libertad) PEOR (sin red de seguridad). El Plan B es su única protección real.

**Para Ivan/Kiki:** todos los docs del prep asumen "Gaby es empleada" — reescritos para la nueva realidad (jun 2026).

Ver análisis completo: `02_MEETINGS/client-prep/roque-meeting/00-CLARIFICACION-LEGAL-INDEPENDIENTE.md`.

### El problema con el dueño (Roque) — actualizado v2:

1. Gaby pidió negociar mejores honorarios
2. Roque respondió: *"En vez de preocuparte por aumentar tu porcentaje, preocupate por aumentar tu productividad"*
3. Implicación: le exige más velocidad, no más pago
4. Cultura: la clínica paga por volumen, no por calidad
5. **Nota del 70% que Gaby mandó quedó SIN RESPUESTA** (audio 22 jun)
6. **Ara (hija) es administradora operativa con poder de veto real** sobre La Merced y otras decisiones de sede

### Estado del conflicto con O3 (a 22 de junio)

| Evento | Estado |
|---|---|
| Reunión inicial (queja paciente + presupuesto) | ✅ Ocurrió (mayo 2026) |
| Roque amenaza con evaluar continuidad | ✅ Confirmado |
| Gaby manda nota pidiendo 70% | ❌ Sin respuesta (silencio) |
| Roque promete 3 meses en La Merced (verbal) | ❌ Cumplida parcialmente |
| La Merced abre con equipo nuevo (sin Gaby) | ✅ Inaugurada (may-jun 2026) |
| Ara (hija de Roque) le dice "no hay lugar" | ✅ Bloqueada (jun 2026) |
| Roque dice "en el próximo período vas a ir" | 🟡 Verbal, sin fecha |
| Próxima reunión con Roque | 🔴 Pendiente, fecha TBD |

### Su frustración:

> Audio: *"Y después viene el dueño y me dice 'aumentá tu productividad'. Y yo le quiero decir: '¿Mi productividad es pensa de qué?'"*

---

## 5. SU ÉTICA Y VALORES — ADN-ALIGNED

### "No paraguayizar" — Tema Central del Audio

**Contexto**: En Paraguay, muchos odontólogos:
- Llevan pacientes de la clínica donde trabajan a su consultorio privado
- Cobran privado sin decirle a la clínica
- Dicen "el seguro no te cubre, te hago descuento"

**Postura de la dra.** (alineada con el ADN):
> Audio: *"Yo nunca hice eso... no me quiero paraguayizar."*

**Por qué no lo hace**:
- Lo ve como "robar" a la clínica
- Tiene valores éticos fuertes
- Prefiere construir legítimamente

**El dilema**:
- El sistema le roba a ella (60k vs. 550k)
- Pero ella no quiere robar a la clínica
- Irónico: los deshonestos tienen razón dentro del sistema perverso

**Implicación estratégica**:
- Necesita construir su propia base, NO depender de pacientes derivados de la clínica
- **No usar la base de datos de O3** (riesgo legal + ético, contradice el ADN)
- Solo usar red personal + referidos

### Lo que el ADN dice sobre el cobro

> *"El paciente es una persona. El trabajo tiene valor. El cobro permite sostener una odontología seria. Humanidad y honorarios no se contradicen. Lo que se rechaza es la estafa, el sobretratamiento y la venta agresiva."*

---

## 6. ASSETS Y RECURSOS

### Lo que tiene:

| Asset | Description | Valor estratégico |
|---|---|---|
| **ADN Profesional firmado** | Documento de 33 páginas con sistema operativo de marca | **MUY ALTO** — single source of truth |
| **Experiencia de 20 años** | Capacidad real de resolver casos complejos | MUY ALTO |
| **Base de datos pacientes** | Miles de pacientes en el sistema de Odontología 3 | ALTO — pero **NO transferible legalmente** |
| **Archivo fotográfico clínico** | Casos documentados (mostró imágenes en sesión) | ALTO — para portfolio y marketing |
| **Reputación de confianza** | Ética, detallada, no overtreatment | ALTO |
| **Network profesional** | Colegas que derivan, especialistas contacts | MEDIO |
| **English proficiency** | Atiende extranjeros con éxito | MEDIO — acceso a expats |
| **Sistema de trabajo con IA** | Usa Laia/GPT para análisis y balance | MEDIO — productividad |
| **Diferenciación visual** | Scrubs de colores, "cercano y colorido" | MEDIO — diferenciador visual único |

### Lo que necesita:

| Recurso | Status | Action needed |
|---|---|---|
| Estructura legal propia | No tiene | Abogado/contador — E.A.S. posible |
| Habilitación propia (MSPBS) | No tiene | Ministerio de Salud |
| Espacio físico | No tiene | Evaluar alquileres — Plan B Burucuyá amiga viernes |
| Sistema de agenda | Comparte el de la clínica | Investigar opciones |
| Payment infrastructure | No tiene | Implementar Pagopar/Bancard |
| Sitio web alineado al ADN | En proceso | Reescribir content/{en,es}/*.json con frase guía + pilares |
| Contenido digital ADN-aligned | Pendiente | Serie "Pensar antes de tocar" + pilares visibles |

---

## 7. PLANES INMEDIATOS (v2 — post-audio 22 jun + ADN 28 jun)

### La Merced ya NO es opción (v2)

- La Merced ya está abierta y operativa.
- Ara (hija de Roque) tiene la decisión operativa sobre quién atiende allá.
- Ara le dijo a Gaby explícitamente "no hay lugar" (audio 22 jun).
- **No volver a pedir La Merced.** Es línea muerta.

### Reunión con Roque (próxima, fecha TBD) — OBJETIVO NUEVO v2 + ADN

**Objetivo viejo (v1):** "negociar" La Merced o un sillón en Luque.

**Objetivo nuevo (v2):** **comunicar la decisión** de abrir práctica privada en Luque en paralelo, y acordar términos de convivencia o salida.

**Cambio crítico:** Gaby ya no entra en modo "no me eches". Entra en modo "te informo mi decisión y acordamos cómo seguir".

**Alineación ADN**: el tono de la reunión debe reflejar el "no me importa si te gusta, te informo y ejecuto" — coherente con la categoría "criterio clínico" (Gaby decide, no negocia desde el miedo).

### Plan B (independiente de Roque) — actualizado

1. **Burucuyá (Luque) viernes** — consultorio de la **amiga odontóloga**. Mensaje de WhatsApp listo: `09_TEMPLATES/follow-up-burucuya-amiga-luque.md`.
2. **Luque lunes a la mañana** — espacio adicional, pendiente confirmar contacto.
3. **Clasipar #1596372, Centro Luque** — Gs 3.675M/mes.
4. **Clínica Mburucuyá (Dra. Mariana Brescia)** — Asunción, no Luque. Ver `01_RESEARCH/locations/luque-space-shortlist-v2.md` para el shortlist completo.

### Base de datos:
- Tiene acceso al sistema de Odontología 3
- Miles de pacientes con nombres, teléfonos, historial
- **PERO (v2 + ADN):** la base de O3 es institucional, **NO transferible legalmente**
- **No usar la base para la práctica privada propia** — riesgo legal + ético + contradice el ADN ("construir legítimamente")
- Solo usar red personal + referidos

### Sesión con Kiki:
- Estructurar plan de negocio formal
- Análisis de mercado
- Página web + branding personal (alineado al ADN)
- Primeros pasos antes de abrir oficialmente

### Evaluar alquileres en Luque (v2):
- 6 opciones en shortlist, **Opción 1 priorizada: amiga Burucuyá viernes** (costo probablemente bajo)
- Ver `01_RESEARCH/locations/luque-space-shortlist-v2.md` para detalle

---

## 8. LO QUE EL GRUPO LE RECOMENDÓ

Del audio, el equipo le dijo:

1. *"Vos podrías ser la primera dentista en Paraguay que tipo tiene los precios bien públicos"*
2. *"Es la primera que se anima a hacer eso"*
3. *"Tenés que tener muchos seguidores, hacerte visible"*
4. *"Crear tu página web ahora, no pongas todavía full abierto"*
5. *"Tenés loyalty program: me traés un paciente, te ganás limpieza"*
6. *"Cada paciente que viene, tric, ya le pasás tu link"*

**Reformulación alineada al ADN:**
1. Ser la primera en ofrecer **odontología basada en criterio clínico con pricing público** — sí
2. Ser la primera en animar a **mostrar razonamiento, no solo resultados** — sí
3. **Visibilidad selectiva, no masiva** — contenido con criterio, no para engagement vacío
4. Sitio web con la frase guía como H1, **alineado al ADN completo**
5. Programa de derivación (no "loyalty" genérico) — coherente con "derivar cuando corresponde"
6. Link post-consulta, después de haber generado confianza

---

## 9. SU VISIÓN EXPRESADA — ADN-ALIGNED

> Audio: *"Para lo que yo quiero hacer, para lo que es el deseo que tengo, no me sirve lo que necesito ahora. (...) Me encantaría hacer mucho de esas cosas que quiero hacer, pero como trabajo ahora no me va a dejar hacer nada."*

### Qué quiere:
- Hacer odontología con criterio, sin presión de productividad
- Atender extranjeros que valoren su trabajo
- Eventualmente tener su propio espacio
- No depender de un sistema que la explota
- **Construir una práctica donde el razonamiento clínico tenga valor propio**
- Capturar el valor del criterio como fee separado, no como margen dentro del procedimiento

### Qué no quiere (alineado al ADN):
- Crecer por volumen indiscriminado
- Overtreatment por revenue
- Seguir siendo "empleada" por 13+ años
- "Paraguayizar" (robar pacientes de la clínica)
- Decir que sí a todo para cerrar venta
- Sonar como community manager genérico
- Competir por precio bajo

---

## 10. SU ESTADO EMOCIONAL

> Audio: *"Yo tengo un mundo de cosas en mi cabeza. Ayer estuve horas con IA haciendo todos mis análisis y me quedé así, en depresión por ahí. Me quedé y dije: 'Dios mío, porque en serio es así.' Tantos años estudié."*

### Señales de burnout (v1):
- Estrés acumulado de 13 años
- Síntomas físicos (ampollas en manos y pies durante doctorado)
- Primer balance financiero a los 20 años de carrera
- Momento de claridad: "Hice el clic"
- Necesidad de cambio urgente

### 🆕 Pivote psicológico del 22 junio 2026 (v2 — CRÍTICO)

> Audio 22 jun, textual: *"Ya no soy la misma Gabriela. Ya no entro de tipo ahí no me eches. Ahora mi reunión, la reunión va a ser desde mi lado."*

> Audio 22 jun, textual: *"esto depende bastante — no de lo que diga Roque — sino de hacia dónde voy a ir en el futuro"*

**Lo que esto significa** (alineado con el ADN):

1. **Gaby ya no negocia desde el miedo.** Antes entraba a las reuniones con Roque en modo "ay, no me eches, yo no me quiero ir". Ahora entra en modo "te informo mi decisión, no me importa si te gusta".
2. **La decisión ya está tomada.** Ya no es "evaluar opciones". Es comunicar y ejecutar.
3. **El framing de toda la comunicación cambia.** De "necesito que me autorices" a "esto es un hecho y te lo digo yo".
4. **El plan B deja de ser backup.** Es el plan real, y la reunión es solo el canal formal para comunicarlo.
5. **Coherencia con el ADN:** la nueva Gabriela ejecuta el pilar "pensar antes de tocar" también con su propia vida — decidió, ejecuta, informa.

**Implicación para todos los scripts:** toda la conversación con Roque debe asumir esta nueva Gabriela. No la Gabriela que pide, no la que tiene miedo. La que **informa** y **ejecuta**.

---

## 11. COMPETIDORES MENCIONADOS EN EL AUDIO

| Competidor | Tipo | Notes | Vulnerabilidad |
|---|---|---|---|
| Mastología | Chain | Volume-focused | Sin criterio visible |
| Dental Paraguay SA | Chain | Franquicia dental | Protocol-driven, overtreatment risk |
| Víctor (dentista de Iván) | Private | Atiende empleados de Iván, 50% discount | Modelo de descuento |
| SOCIEDAD PARAGUAYA DE ODONTOPEDIATRÍA | Sociedad profesional | Tiene lista de precios pública | Sin diferenciación clínica |
| Círculo Odontológico Paraguayo (COP) | Sociedad profesional | Arancel de referencia | Pricing reference, no marca |
| Otros dentistas privados | Multi | Pricing Gs 180-550k por caries | Posicionamiento genérico |

**Cómo nos posicionamos (ADN-aligned):**
- vs. chains: personalizado, caso por caso, criterio primero
- vs. franchises: conservador, criterios primero, decir que no cuando corresponde
- vs. private genérico: planificación como servicio + razonamiento visible
- vs. discount-driven: no competimos por precio, competimos por criterio

---

## 12. DATOS FALTANTES (PARA VALIDAR)

| Dato | Por qué importa | Cómo obtener |
|---|---|---|
| Revenue mensual exacto | Calibrar modelo financiero | Balance real de la dra. |
| Costos mensuales (materiales, travel) | Calcular margen real | Historial + cálculo |
| Contrato con Odontología 3 | Entender obligaciones | Pedir copia |
| Pricing de alquileres reales | Evaluar viabilidad | Cotizaciones |
| Opinión de pacientes actuales | Validar posicionamiento | Entrevistas |
| Confirmar espacio amiga Burucuyá | Activar Plan B | Visita física + habilitar EAS |

---

## 13. UPDATED SWOT (alineado al ADN)

### Fortalezas:
- 20 años de experiencia comprobada
- ADN Profesional firmado (sistema operativo de marca)
- Reputación de ética y calidad
- Capacidad para casos complejos (le encantan los "desastres")
- Archivo fotográfico clínico disponible
- English proficiency
- Sistema de trabajo con IA
- Diferenciación visual (scrubs de colores)

### Debilidades:
- 13 años sin cuestionar el modelo (hasta ahora)
- Primer balance financiero recién a los 20 años
- Dependiente de la clínica para todo (cambia con Plan B)
- Sin estructura legal propia
- Sin presencia digital ADN-aligned (en proceso)
- Sin payment infrastructure
- Dilemas éticos sobre uso de base de datos (resueltos: no usar)

### Oportunidades:
- Vacío: nadie ofrece "odontología basada en criterio clínico" en PY
- Pricing premium para expats
- Primera en mostrar razonamiento + precios públicos
- Alquiler en Luque (cerca de casa, menos commute)
- Payment plans via Pagopar para casos grandes
- Programa de derivación (alineado a "derivar cuando corresponde")
- **🆕 Amiga de Burucuyá (Luque) como Plan B firme (v2):** espacio viernes ya casi confirmado
- **🆕 Diferenciación visual de marca (v2):** scrubs de colores, paleta cálida — diferenciador único

### Amenazas:
- Roque puede reaccionar si detecta plan B (mitigado: status independiente)
- Ara como veto holder (mitigado: plan B independiente)
- Sistemas de seguro aumentando presión
- Chains expandiendo en Asunción
- Competencia de dentistas que sí "paraguayizan" (Gaby no, por convicción)
- Contexto económico PY (inflación, etc.)
- Regulatory risk si hace algo incorrecto con base de datos (mitigado: no la usa)

---

## 14. PRÓXIMOS PASOS IDENTIFICADOS

### Fase 0: Baseline (Esta semana)
- [ ] Grabar audio #1 (cronología Roque 4 semanas) y audio #2 (números reales)
- [ ] Reunión con Roque — 🆕 **objetivo v2: comunicar decisión, no negociar La Merced**
- [ ] Cotizar alquileres en Luque — 🆕 **Opción 1 priorizada: amiga Burucuyá viernes**
- [ ] Balance financiero real (no estimate)

### Fase 0.5: Post-reunión Roque (1-2 semanas)
- [ ] Si Roque dice sí + Ara avala → borrador de acuerdo, firma
- [ ] Si Roque dice no / Ara bloquea → activar Plan B completo (Burucuyá amiga viernes)
- [ ] Si reunión sale mal → plan de emergencia 7 días (`13-emergency-plan-7-days.md`)

### Fase 1: Activación Plan B (2-4 semanas)
- [ ] Confirmar espacio amiga Burucuyá (visita física + habilitar EAS)
- [ ] Validar pricing con mystery shopping
- [ ] Hablar con Pagopar sobre implementación
- [ ] Consultar abogado sobre estructura legal
- [ ] Evaluación de equipos necesarios
- [ ] Reescribir contenido del sitio web alineado al ADN (en proceso)

### Fase 2: ADN-aligned public launch (1-2 meses)
- [ ] Sitio web con frase guía + 5 pilares + símbolo + manifiestos
- [ ] Branding visual (paleta cálida, scrubs coloridos)
- [ ] Serie insignia "Pensar antes de tocar" — primeros 5 episodios
- [ ] Implementar Google Business optimizado
- [ ] Pricing público (alineado a canonical-pricing-reference-v2.md)

### Fase 3: Growth (3-6 meses)
- [ ] Opción B ejecutándose (Burucuyá amiga viernes → Luque lunes)
- [ ] Red de derivación activa (alineada a "derivar cuando corresponde")
- [ ] Programa de referidos
- [ ] Evaluar Opción C (clínica propia) para Year 2

---

**Document status**: ACTUALIZADO con ADN Profesional del 28 jun 2026
**Cambios v3:** Alineación completa con ADN. Reemplaza secciones de filosofía clínica, valores y visión para que reflejen los 5 pilares del ADN. Status legal independiente explícito. Datos duros preservados (audio 22 jun, 70% sin respuesta, Ara veto holder, Plan B Burucuyá).
**Cambios v2:** Ara como decision-maker real + pivote psicológico + nota 70% sin respuesta + La Merced cerrada + amiga Burucuyá como Plan B firme
**Next update**: Después de la próxima reunión con Roque (TBD) + después de recibir audios #1 y #2 de Gaby